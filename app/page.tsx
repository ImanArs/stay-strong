...
export default function App() {
  ...
  const [currentChallenge, setCurrentChallenge] = useState<Exercise | null>(null)
  ...
  useEffect(() => {
    ...
    setCurrentChallenge(jacobChallenges[Math.floor(Math.random() * jacobChallenges.length)])
  }, [])
  ...
  const saveUserRecord = (record: UserRecord) => {
    ...
    const existingIndex = updatedRecords.findIndex((r) => r.exerciseId === record.exerciseId)
  ...
  const completeFight = (won: boolean) => {
    if (currentChallenge) {
      const historyItem: HistoryItem = {
        ...
        userTime: won ? currentChallenge.jacobTime - 5 : currentChallenge.jacobTime + 10,
  ...
  if (showOnboarding) {
    return <Onboarding onComplete={completeOnboarding} onSkip={completeOnboarding} />
  }
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-b from-gray-900 to-gray-950 text-white">
      <AnimatePresence mode="wait">
        {showFightMode ? (
          <FightMode exercise={currentChallenge!} onComplete={completeFight} onCancel={() => setShowFightMode(false)} />
        ) : (
          <main className="flex-1 overflow-y-auto pb-16">
            {activeTab === "home" && (
              <HomeScreen history={history} currentChallenge={currentChallenge!} onAcceptChallenge={acceptChallenge} />
            )}
            {activeTab === "record" && (
              <CreateRecord exercises={exercises} userRecords={userRecords} onSaveRecord={saveUserRecord} />
            )}
